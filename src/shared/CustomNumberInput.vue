...
function handleInput() {
  ...
  if (
    ...
    input.value.value = input.value.value.substr(1, input.value.value.length);
  }
...
function checkInput() {
  ...
  if (props.min !== undefined && parseFloat(input.value!.value) < props.min) {
    value.value = props.min;
    input.value!.value = props.min.toString();
  }

  if (props.max !== undefined && parseFloat(input.value!.value) > props.max) {
    value.value = props.max;
