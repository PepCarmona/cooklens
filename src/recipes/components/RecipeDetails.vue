  ...
  <div class="container" :class="{ thin: !!id, embedded }" v-else>
    ...
    <template v-else>
      ...
      <div class="brief" :class="{ 'no-image': !recipeHasImages }">
        ...
        <div v-if="recipe.authorDetails" class="author">
          By: {{ recipe.authorDetails.username }}
        </div>
...
<script lang="ts">
export function getFormattedTitle(recipe: ClientRecipe): string {
...
import { Ingredient, ClientRecipe } from '../../../types';
...
function getRecipeDetails() {
  ...
  recipeService.getRecipe(id).catch((err: any) => {
    console.error(err);
