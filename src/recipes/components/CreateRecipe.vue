          ...
          <template v-if="showTab === 'introduction'">
            ...
            <div class="row">
              <CustomInput
                ...
                :ref="(el) => textAreaRefs.push(el as InstanceType<typeof CustomInput>)"
              />
            </div>
              ...
              <div class="row mt-0" ref="ingredientInput">
                ...
                >
                  ...
                  <CustomInput
                    ...
                    type="text"
                    :label="
                      showAdvancedIngredientsForm ? 'Ingredient' : undefined
                    "
                    :placeholder="
...
const emit = defineEmits(['newRecipeSaved', 'go-back']);
...
const newRecipe = reactive<ClientRecipe>(new RecipeClass()) as ClientRecipe;
...
const sanitizedRecipe = computed<ClientRecipe>(() => {
...
async function addStep() {
  ...
  if (
    ...
    newRecipe.instructions[newRecipe.instructions.length - 1].content.length > 0
  ) {
  ...
  textAreaRefs.value[textAreaRefs.value.length - 1].$el.focus();
}
...
async function importRecipe(value: ClientRecipe) {
  ...
  textAreaRefs.value.forEach((textArea) => textArea.$el.resizeTextArea(null));
}
